  ...
  return (
    <article className="mx-auto max-w-3xl px-6 py-16">
      ...
      {attachments.length > 0 && (
        <div className="mt-8">
          <h2 className="text-lg text-ink-900">Attachments</h2>
          <ul className="mt-2 flex flex-col gap-1">
            {attachments.map((asset) => (
              <li key={asset.id}>
                <a href={asset.fileUrl} target="_blank" rel="noreferrer" className="text-ink-800 underline">
                  {asset.altText}
                </a>
              </li>
            ))}
      ...
      {item.relatedLinks && item.relatedLinks.length > 0 && (
        <div className="mt-8">
          <h2 className="text-lg text-ink-900">Related links</h2>
          <ul className="mt-2 flex flex-col gap-1">
            {item.relatedLinks.map((link, i) => (
              <li key={i}>
                <a href={link.url} target="_blank" rel="noreferrer" className="text-ink-800 underline">
                  {link.label}
                </a>
              </li>
            ))}
